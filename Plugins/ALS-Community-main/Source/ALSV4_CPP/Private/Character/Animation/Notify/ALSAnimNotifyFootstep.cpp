...
//---------- Edit Noise
	...
	if (MeshOwner->GetClass()->ImplementsInterface(UALSFootstepNoiseInterface::StaticClass()))
	{
		const FVector Location = MeshComp->GetComponentLocation();
		const float Loudness = 1.0f; //ToDo

		IALSFootstepNoiseInterface::Execute_HandleALSFootstepNoise(
			MeshOwner,
			Location,
			Loudness
		);
